...
TaskSpaceOffline::TaskSpaceOffline()
{
    ...
    while (_walkstate==true) {
        MatrixXd m=AnkleTrajectory(time);
        double m1=m(0,0);
        double m2=m(1,0);
        double m3=m(2,0);
        double m4=m(3,0);
        ...
        LeftFootXTrajectory.append(m1);
        LeftFootYTrajectory.append(m2);
        LeftFootZTrajectory.append(m3);
        RightFootXTrajectory.append(m4);
        RightFootYTrajectory.append(m5);
        RightFootZTrajectory.append(m6);


        MatrixXd P=PelvisTrajectory (time);
        CoMXVector.append(P(0,0));
        CoMYVector.append(P(1,0));
        CoMZVector.append(P(2,0));

        CoMXVelocityVector.append(P(3,0));
        CoMYVelocityVector.append(P(4,0));
        CoMZVelocityVector.append(P(5,0));


        CoMXAccelerationVector.append(P(6,0));
        CoMYAccelerationVector.append(P(7,0));
        CoMZAccelerationVector.append(P(8,0));
        ...
        if (round(time)==round(MotionTime)){
            return;
        }
...
void TaskSpaceOffline::SetParameters(){
    ...
    ReferencePelvisHeight=0.8;
    MaxHeightPelvis=MinHeightPelvis+Delta;
    if (DesiredVelocity==0.20) {
        Xe=1.7*Sc*StepLength/(Rse+1);
        Xs=0*Rse*Xe;
        YpMax=1.1*Rm*0.5*_pelvisLength;
        Yd=1.1*Rd*YpMax;
        YStMax=1.0*YpMax;
